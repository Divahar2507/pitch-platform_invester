...
export const aiService = {
  analyzeStartup: async (startup: Startup): Promise<string> => {
    const ai = getAI();
  ...
  getMarketInsight: async (portfolio: Startup[]): Promise<string> => {
    const ai = getAI();
  ...
  chat: async (history: {role: string, content: string}[], message: string) => {
    const ai = getAI();
